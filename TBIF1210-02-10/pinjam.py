...
def cek_borrow_history(gadget_id, databases): #mengecek apakah user pernah meminjam gadget yg sama, menghasilkan true jika sudah pernah meminjam
    id = databases[active_account][0]
    db = databases[gadget_borrow_history]
    if len(db) > 1:
        for row in range(len(db)):
            if int(id) == db[row][1] and gadget_id == db[row][2] and db[row][5] == "False":
                return True
    return False
