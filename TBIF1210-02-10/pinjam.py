...
# mengecek apakah user pernah meminjam gadget yg sama
def cek_borrow_history(gadget_id, data):
    id = data[active_account][0]
    f = data[gadget_borrow_history]
    for row in range(len(f)):
        if int(
                id) == f[row][1] and gadget_id == f[row][2] and f[row][5] == False:
            return True
    return False
            ...
            if id[0] == "G":
                if cek_id(id, databases, type):
                    if cek_borrow_history(id, databases):
                        ...
                        id_peminjam = user[0]
                        is_returned = False
                        new_history = [str(id_pinjam), int(
                            id_peminjam), id, tanggal, jumlah, is_returned]
                        history.append(new_history)
                        # output
                        print("Item", name, "(" + str(jumlah) +
                              ") telah berhasil dipinjam!")
                else:
